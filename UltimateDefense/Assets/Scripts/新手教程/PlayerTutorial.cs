...
public class PlayerTutorial : MonoBehaviour
{
                ...
                ShowDialog("在这里，你可以选择不同的增强Buff。\r\n有时候还能遇到折扣惊喜。\r\n如果两个Buff相同，必定打折。", DialogPositionType.Middle, () =>
                {
                    ...
                    ShowDialog("当前Buff若都已购买，\r\n并且你有足够的金币时，\r\n可以刷新一批新的选项。", DialogPositionType.Middle, () =>
                    {
                        ShowDialog("敌人马上就要来了。\r\n总共 20 波……\r\n可能很艰难，但我相信你能撑住。", DialogPositionType.Middle, () =>
                        {
                            ...
                            HideArrow();
                            HideDialog();
                            DataManager.Instance.PlayerInfo.Config["NewPlayer"] = 0;
    ...
    {
        //判断是否有这个字段
        if (!DataManager.Instance.PlayerInfo.Config.ContainsKey("NewPlayer"))
        {
            if (DataManager.Instance.PlayerInfo.TotalPassCount.Value > 0)//已经通关过
            {
                DataManager.Instance.PlayerInfo.Config["NewPlayer"] = 0;//不是新手
            }
            else
            {
                DataManager.Instance.PlayerInfo.Config["NewPlayer"] = 1;//是新手
            }
        }
