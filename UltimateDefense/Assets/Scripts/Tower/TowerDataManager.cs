...
public class TowerDataManager : ManagerBase<TowerDataManager>   
{
    ...
    public override void Init()
    {
        ...
        TowerDiscription[TowerType.Ricochet] = $"子弹对第一个敌人造成<color=#F4C760>{GetTowerData(TowerType.Ricochet).BaseDamage /10f}</color>点伤害，额外弹射3个敌人,每次弹射伤害衰减25%\r\n\r\n被动：每消灭3个敌人为城墙恢复1点生命值";
        TowerDiscription[TowerType.Spread] = $"每次并排发射三颗子弹，每颗子弹伤害为<color=#F4C760>{GetTowerData(TowerType.Spread).BaseDamage /10f}</color>，初始暴击率翻倍\r\n被动：每次暴击为城墙恢复1点血";
        TowerDiscription[TowerType.Piercing] = $"子弹初始伤害为<color=#F4C760>{GetTowerData(TowerType.Piercing).BaseDamage / 10f}</color>，可以穿透敌人\r\n被动：每获得100金币为城墙恢复1点生命\r\n";
    ...
    public static int GetBaseDamage(TowerType towerType,int level)
    {
        return towerType switch
        {
            ...
            TowerType.Ricochet => 8 + (level - 1) * 2,//max为27，衰减为25%
            TowerType.Spread => 5 + (level-1) * 2,//max为23
            TowerType.Sniper => 15 + (level - 1) * 6,//max为69
    ...
    public static int GetBaseCap(TowerType towerType, int level)
    {
        return towerType switch
        {
            TowerType.Basic => 10,// + (level - 1) * 1,
            TowerType.RapidFire => 20,// + (level - 1) * 1,
            TowerType.Ricochet => 10,// + (level - 1) * 4,
            TowerType.Spread => 15,// + (level - 1) * 4,
            TowerType.Sniper => 5,// + (level - 1) * 4,
            TowerType.Piercing => 10,// + (level - 1) * 4,
            _ => 0
        };
    }
    ...
    public static float GetBaseAtkRate(TowerType towerType, int level)
    {
        return towerType switch
        {
            TowerType.Basic => 1.0f,// + (level - 1) * 0.1f,
            TowerType.RapidFire => 1.0f,// + (level - 1) * 0.1f,
            TowerType.Ricochet => 1.0f,// + (level - 1) * 4,
            TowerType.Spread => 1.0f,// + (level - 1) * 4,
            TowerType.Sniper => 0.7f,// + (level - 1) * 4,
            TowerType.Piercing => 1.0f,// + (level - 1) * 4,
            _ => 0
        };
    }
    ...
    public static float GetBaseCritProb(TowerType towerType, int level)
    {
        return towerType switch
        {
            ...
            TowerType.Spread => 0.2f,// + (level - 1) * 4,
    ...
    public static float GetBaseCritMult(TowerType towerType, int level)
    {
        return towerType switch
        {
            ...
            TowerType.Sniper => 1.5f,// + (level - 1) * 4,
