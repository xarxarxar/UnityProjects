...
public class CollectionManager : ManagerBase<CollectionManager>
{
    ...
    private void OnEnemyDie(Enemy enemy)
    {
        ...
        probability += DataManager.Instance.PlayerInfo.TotalPassCount.Value * 0.001f;
        probability += WaveManager.Instance.CurrentRound * 0.0003f;
        probability = Mathf.Min(probability, 0.015f);
        probability += DataManager.Instance.PlayerInfo.BattleCountNoCollect.Value * 0.0005f;
        Debug.Log($"敌人死亡, probability = {probability}");

        if (Random.value < probability)
        {
            int idx = GetRandomCharacter();
            DataManager.Instance.PlayerInfo.BattleCountNoCollect.Value = 0;
    ...
    private void OnEndBattle(bool isSuccess)
    {
        foreach (var kv in BattleManager.Instance.CollectionSignleBattle)
        {
            DataManager.Instance.PlayerInfo.MonthlyCollection[kv.Key] += kv.Value;
        }
        DataManager.Instance.SavePlayerInfo();
    }
}
