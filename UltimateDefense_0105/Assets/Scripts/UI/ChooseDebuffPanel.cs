...
public class ChooseDebuffPanel : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (DataManager.Instance.PlayerInfo.TotalPassCount == 0)//如果通关次数为0，则直接跳过这一步
        {
        ...
        _debuff = new Debuff();
        _chooseCount = 0;
        _titleText.text = $"您已通关了{DataManager.Instance.PlayerInfo.TotalPassCount}次\r\n需抽取{DataManager.Instance.PlayerInfo.TotalPassCount}个Debuff再进行挑战";
    ...
    private IEnumerator ChooseDebuffsCoroutine()
    {
        ...
        for (int i = 0; i < DataManager.Instance.PlayerInfo.TotalPassCount; i++)
        {
