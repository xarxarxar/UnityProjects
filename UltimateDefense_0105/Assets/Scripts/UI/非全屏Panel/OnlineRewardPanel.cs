...
public class OnlineRewardPanel : BasePanel
{
    ...
    public override void Init()
    {
        ...
        DataManager.Instance.PlayerInfo.DailyTask.OnOnlineMinutesChanged += (oldValue,minutes) =>
        {
        ...
        for(int i = 0; i < taskRewards.Count; i++)
        {
            ...
            if (i== taskRewards.Count - 1)
            {
                taskRewards[i].Init(RewardType.Crown, 1, description,
                    _needMinutes[i], DataManager.Instance.PlayerInfo.DailyTask.OnlineMinutes,
            ...
            else
            {
                taskRewards[i].Init(RewardType.Diamond, _needMinutes[i]/2,
                    ...
                    DataManager.Instance.PlayerInfo.DailyTask.OnlineMinutes,
                    DataManager.Instance.PlayerInfo.DailyTask.RewardReceived[description],
                    (taskTag) =>
                    {
                        DataManager.Instance.PlayerInfo.DailyTask.SetRewardReceived(taskTag, true);
