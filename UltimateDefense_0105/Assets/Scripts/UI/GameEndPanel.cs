...
public class GameEndPanel : MonoBehaviour
{
    ...
    private void DoubleButton()
    {
        WeChatManager.ShareApp(() =>
        {
            ...
            DataManager.Instance.PlayerInfo.DailyTask.SetShareCount(DataManager.Instance.PlayerInfo.DailyTask.ShareCount+1);
            //王冠和钻石的显示数量翻倍
            ...
            for(int i=0;i<signleCollection.Count;i++)
            {
                ...
                int oldValue = DataManager.Instance.PlayerInfo.MonthlyCollection[signleCollection[i].index];
                DataManager.Instance.PlayerInfo.SetMonthlyCollection(signleCollection[i].index, oldValue + signleCollection[i].Count);
            }
            //再额外给予玩家一倍的奖励
