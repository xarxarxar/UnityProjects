...
public class Upgrade_IncreaseTowerBulletCap : UpgradeBase
{
    ...
    {
        return TowerManager.Instance.CurrentTower.BulletCap < 50f;//炮塔最大的弹夹容量不超过50
    }
    ...
    {
        // 通知 Manager 保存全局加成
        TowerManager.Instance.CapBuff.AddChain.AddBuff(new BuffModifier { Value = _bonus, Source = this });
    }
