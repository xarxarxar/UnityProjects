...
public class Upgrade_IncreaseTowerAttackRate : UpgradeBase
{
    ...
    public static Upgrade_IncreaseTowerAttackRate CreateDynamicUpgrade()
    {
        ...
        int cost = Mathf.RoundToInt(bonus * (100 + UpgradeManager.Instance.RefreshCount.Value * 50));
    ...
    {
        return TowerManager.Instance.CurrentTower.AttackRate < 2.5f;//每秒设计次数不超过2.5次
    }
    ...
    {
        // 通知 Manager 保存全局加成
        TowerManager.Instance.AtkRateBuff.AddChain.AddBuff(new BuffModifier
        {
            Value = _attackRateBonus / 1.5f,
            Source=this,
        });
    }
