...
{
    private static List<ManagerBase> managers = new List<ManagerBase>();//所有的Manager类
    ...
    {
        // 过滤符合条件的 Manager，并且 Index != -1
        var managersToInit = managers
            .Where(m => m.Stage == initStage && m.Index != -1)
            .OrderBy(m => m.Index); // 按 Index 升序排列

        foreach (var m in managersToInit)
        {
            m.Init();
        }
    }
