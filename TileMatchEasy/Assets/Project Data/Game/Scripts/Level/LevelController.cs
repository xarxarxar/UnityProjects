...
namespace Watermelon
{
    public class LevelController : MonoBehaviour
    {
        ...
        // ----- Endless mode (single level, no progression) -----
        private const int ENDLESS_GRID_SIZE = 6;
        private const int ENDLESS_START_TILE_TYPES = 10;
        private const int ENDLESS_MAX_TILE_TYPES = 19;
        private const int ENDLESS_SCORE_PER_MATCH = 10;
        private const float ENDLESS_TIME_PER_MATCH = 1f;

        private static int endlessTileTypesCount;
        private static int endlessScore;
        public static int EndlessScore => endlessScore;
        private static LevelData endlessLevelData;
        ...
        {
            DockBehavior.MatchCombined -= OnDockMatchCombined;
        }
        ...
        public void Initialise()
        {
            ...
            // Validate saved level data: if it is out of the current level database range
            // (e.g. leftover data from an old project), reset it back to level 1.
            int maxLevelIndex = database.AmountOfLevels - 1;
            if (levelSave.MaxReachedLevelIndex > maxLevelIndex || levelSave.DisplayLevelIndex > maxLevelIndex
                || levelSave.MaxReachedLevelIndex < 0 || levelSave.DisplayLevelIndex < 0)
            {
                ...
                Debug.Log("[LevelController]: saved level data was out of range and has been reset to level 1");
            }
            ...
            // Endless mode: build a runtime 6x6 grid level and listen for match events.
            endlessLevelData = CreateEndlessLevelData();
            DockBehavior.MatchCombined += OnDockMatchCombined;
        ...
        public void OnMatchCompleted()
        {
            ...
            {
                // Endless mode: the whole board was cleared - refresh it with one more tile type.
                RefreshEndlessBoard();
        ...
        public void LoadEndlessLevel()
        {
            ...
            endlessScore = 0;
            endlessTileTypesCount = ENDLESS_START_TILE_TYPES;

            // Countdown timer is always active in endless mode (fresh reset).
            timerSliderObject.SetActive(false);
            timerSliderObject.SetActive(true);

            SpawnEndlessBoard(true, null);
        }

        public void RefreshEndlessBoard()
        {
            if (levelRepresentation != null)
                UnloadLevel();

            endlessTileTypesCount = Mathf.Min(endlessTileTypesCount + 1, ENDLESS_MAX_TILE_TYPES);

            SpawnEndlessBoard(false, null);
        }

        private void SpawnEndlessBoard(bool reloadBackground, SimpleCallback onLevelLoaded)
        {
            ...
            {
                gameUI.PowerUpsUIController.OnLevelStarted(0);
                gameUI.UpdateLevelNumber(endlessScore);
            }
            ...
            TileData[] availableObjects = GetEndlessAvailableTiles(endlessTileTypesCount);
        ...
        private TileData[] GetEndlessAvailableTiles(int count)
        {
            TileData[] allTiles = database.Tiles;
            int cappedCount = Mathf.Clamp(count, 1, allTiles.Length);

            TileData[] result = new TileData[cappedCount];
        ...
        private void OnDockMatchCombined(List<ISlotable> matchedElements)
        {
            if (!GameController.IsGameActive) return;

            // +10 score and +1 second of countdown time for every 3-match.
            endlessScore += ENDLESS_SCORE_PER_MATCH;

            TimeSliderControl.AddTime(ENDLESS_TIME_PER_MATCH);
        ...
        private LevelData CreateEndlessLevelData()
        {
            LevelData levelData = ScriptableObject.CreateInstance<LevelData>();

            Layer filledLayer = CreateEndlessLayer(true);
            Layer emptyLayer = CreateEndlessLayer(false);

            ReflectionUtils.InjectInstanceComponent<LevelData>(levelData, "layers", new Layer[] { filledLayer, emptyLayer });
            ReflectionUtils.InjectInstanceComponent<LevelData>(levelData, "bottomLayerWidth", ENDLESS_GRID_SIZE);
            ReflectionUtils.InjectInstanceComponent<LevelData>(levelData, "bottomLayerHeight", ENDLESS_GRID_SIZE);
            ReflectionUtils.InjectInstanceComponent<LevelData>(levelData, "elementsPerLevel", ENDLESS_START_TILE_TYPES);
            ReflectionUtils.InjectInstanceComponent<LevelData>(levelData, "coinsReward", 0);

            return levelData;
        }

        private Layer CreateEndlessLayer(bool filled)
        {
            LayerRow[] rows = new LayerRow[ENDLESS_GRID_SIZE];

            for (int y = 0; y < ENDLESS_GRID_SIZE; y++)
            {
                CellData[] cells = new CellData[ENDLESS_GRID_SIZE];

                for (int x = 0; x < ENDLESS_GRID_SIZE; x++)
                {
                    cells[x] = new CellData();
                    cells[x].IsFilled = filled;
                    cells[x].Effect = TileEffectType.None;
                }

                LayerRow row = new LayerRow();
                ReflectionUtils.InjectInstanceComponent<LayerRow>(row, "cells", cells);

                rows[y] = row;
            }

            Layer layer = new Layer();
            ReflectionUtils.InjectInstanceComponent<Layer>(layer, "rows", rows);
