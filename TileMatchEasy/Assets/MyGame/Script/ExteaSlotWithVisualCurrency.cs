...
{
    public class ExteaSlotWithVisualCurrency : MonoBehaviour, IIAPStoreOffer
    {
        ...
        [Space]
        [SerializeField] string description = "增加一个存放位置，\r\n一局最多使用一次"; // 显示描述的文本
        ...
        {
            // 从保存控制器获取计时器状态
            save = SaveController.GetSaveObject<SimpleLongSave>("ExtraSlot With Share");
        ...
        private void Update()
        {
            ...
            {
                canShare = false;//默认不能分享
            }
            else // 如果计时未结束
            {
                canShare = false;
            }
            priceText.text = canShare ? "分享" : price.ToString(); // 显示文本
            shareImage.SetActive(canShare);
            currencyImage.SetActive(!canShare);
        }
        ...
        private void AddPowerUp()
        {
            ...
            if (canShare)
            {
                CallWechat.ShareApp(() =>
                {
                    ...
                    {
                        PUController.AddPowerUp(PUType.ExtraSlot, 1);//添加一个道具的事例
                    });
                });
            }
            else
